...
import { displayDeleteStore } from "../../../../redux/display";
import { APIS, requestJwt } from "../../../../_services";
import { setAlert } from "../../../../redux/snackbar";

const DeleteStore = (props) => {
  const { account, getStore} = props
  const myRef = useRef();
  ...
  const closeDialog = async() =>{
    dispatch(displayDeleteStore("none"))
  }
  return (
    <>
      <div className="modal" style={{ display: `${display}`, zIndex: 1000 }}>
        <div className="modal__content" ref={myRef}>
          ...
          <div className="model-title">
            <h2 className="num model-title__header">Delete store</h2>
          </div>
          <form className="model-form" onSubmit={ deleteUser }>
