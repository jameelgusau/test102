...
import { setMyCertificates } from "../../../../redux/myCertificates";
...
const MyCertificates = () => {
  ...
  const { totalRecord, records, totalPages, currentPage } = useSelector((state) => state.mycertificates.value);
        ...
        >
          <MenuItem
            onClick={() => {
              ...
              dispatch(displayViewMyCertificates("block"));
              // setAnchorEl(null);
            }}
          >
            View Certificate
  ...
  const getCertificates = async (data) => {
    ...
    const getUs =  async () =>{
      try{
        ...
        if(response?.data){
        dispatch(setMyCertificates(response?.data?.data))}
      }catch(err){
